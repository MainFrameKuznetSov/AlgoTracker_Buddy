...
const MistakesByProblem = () => {
  ...
  const [mistakes, setMistakes] = useState<MistakeByProblem[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();

  const fetchMistakesByProblem = async () => {
    if (!problemName.trim()) {
      toast({
        title: "Problem name required",
        description: "Please enter a problem name to search for.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `Found ${data.length} mistakes for problem: ${problemName}`,
      });
    } catch (error: any) {
      toast({
        title: "Error",
        description: error?.response?.data?.detail || "Failed to fetch mistakes by problem. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
          ...
          <div className="flex space-x-4">
            <div className="flex-1">
              ...
              <Input
                ...
                placeholder="Enter problem name (e.g., Two Sum, Binary Search)"
                onKeyPress={(e) => e.key === 'Enter' && fetchMistakesByProblem()}
              />
            </div>
            <div className="flex items-end">
              <Button onClick={fetchMistakesByProblem} disabled={isLoading}>
                {isLoading ? (
...
export default MistakesByProblem;