...
const ContestSubmissions = () => {
  ...
  const fetchLiveMistakes = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/contests/${handle}`,
  ...
  const handleAddMessage = (submission: ContestSubmission) => {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            ...
            <span>Contest Submission Tracker</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex space-x-4">
            ...
            <div>
            <Label htmlFor="contestId">Contest Number</Label>
            <Input
                id="contestId"
                type="number"
                value={contestId}
